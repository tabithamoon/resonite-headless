...
cd ${STEAMAPPDIR}/Headless/Libraries && \
	curl -s https://api.github.com/repos/resonite-modding-group/ResoniteModLoader/releases/latest | \
	grep "browser_download_url.*ResoniteModLoader.dll" | \
	cut -d : -f 2,3 | tr -d \" | \
	xargs curl -s -L --output ResoniteModLoader.dll
	
cd ${STEAMAPPDIR}/Headless/rml_libs && \
	curl -s https://api.github.com/repos/resonite-modding-group/ResoniteModLoader/releases/latest | \
	grep "browser_download_url.*0Harmony.dll" | \
	cut -d : -f 2,3 | tr -d \" | \
	xargs curl -s -L --output 0Harmony.dll
